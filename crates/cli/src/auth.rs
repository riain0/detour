...
pub fn resolve_auth_token(auth_mode: &AuthMode, broker_url: &str) -> anyhow::Result<Option<String>> {
    match auth_mode {
        ...
        AuthMode::GcpOidc => Ok(Some(get_gcp_identity_token(&identity_audience(broker_url))?)),
    }
}
